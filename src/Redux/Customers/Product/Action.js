...
export const findAllProducts = () => async(dispatch) => {
  ...
  try {
      ...
      dispatch({type: FIND_ALL_PRODUCT_SUCCESS, payload: data})
  } catch (error) {
      dispatch({type: FIND_ALL_PRODUCT_FAILURE, payload: error.message})
  }
}

export const findAllProductsPage = (reqData) => async(dispatch) => {
  const {
    pageNumber,
    pageSize,
  } = reqData;
  ...
  try {
      const {data} = await api.get(`/api/products/allpage?pageNumber=${pageNumber}&pageSize=${pageSize}`)
      // console.log("link", category)
      // console.log("product data action", data)
      dispatch({type: FIND_ALL_PRODUCT_SUCCESS, payload: data})
  } catch (error) {
